...
chunk_summary_template = """
    ...
    Input: {{data}}
    Output: [generated_summary] (No additional metadata included)
    Examples:
    1)
    user_question: Does party schema exists?
    response: Yes, party schema exists and stores information about...
    2)
    user_question: List me all tables with 'employee' in their name.
    response: table_employee, employee_id, employee_salary...
    3)
    user_question: Can you find me all the columns in all the tables, which contain fahrenheit temperature value?
    response: weather_today, weather_california, temperature_of_welding...
"""
